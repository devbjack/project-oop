...
public class SchoolManagement {
    ...
    void caseA(){
        ...
        while(loop){
            ...
            switch(choose){
                case 1 -> bus = caseAChoose1(bus);
                case 2 -> caseAChoose2(bus);
                case 3 -> caseAChoose3(bus);
                case 4 -> loop = false;
            }
        }
    }
    Bus caseAChoose1(Bus bus){
        int busId = 2;
        int driverId = 2;
        int busNumber = 2;
        int capacity = 2;
        System.out.println("Bus added successfully.");
        return new Bus(busId, driverId, busNumber, capacity);
    }
    void caseAChoose2(Bus bus){
        if (bus == null) return;
        String busDetails = bus.busDetails(); 
        System.out.println(busDetails);
    }
    void caseAChoose3(Bus bus){
        if (bus == null) return;
        String showSeats = bus.showSeats();
        int busId = bus.getBusId();
        System.out.printf("The total seat for %d is %s", busId, showSeats);
    }
    ...
    void caseF(){
        boolean loop = true;
        Auditorium auditorium = new Auditorium(3);  //, i think we have insintionlaz this class
        while(loop){
            System.out.println("[F] Auditorium");
